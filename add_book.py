from tkinter import *
import pandas as pd

path = "C:/Users/LENOVO/Desktop/Notes&Codes/Python_Projects/Library Management System/Library_info.csv"
df = pd.read_csv(path)

my_root = Tk()
my_root.title("Add a Book")
my_root.geometry("400x400")
my_root.config(bg="Black")

l1 = Label(text = "Add a Book", bg = "Black", fg= "Yellow")
l1.config(font=("Arial",20,"bold italic"))
l1.pack(pady= 15)

l2 = Label(text = "Book Name", bg = "Black", fg= "Yellow")
l2.config(font=("Arial",10,"bold italic"))
l2.pack(pady= 15)

e1 = Entry()
e1.pack()

l3 = Label(text = "Book id", bg = "Black", fg= "Yellow")
l3.config(font=("Arial",10,"bold italic"))
l3.pack(pady= 15)

e2 = Entry()
e2.pack()

l4 = Label(text = "Author Name", bg = "Black", fg= "Yellow")
l4.config(font=("Arial",10,"bold italic"))
l4.pack(pady= 15)

e3 = Entry()
e3.pack()


def submit():
    row = [pd.Series([e1.get(), e2.get(), e3.get()], index=df.columns)]
    n_df = df.append(row)
    n_df.to_csv(path, index=False)
    sl = Label(text = "Submitted Successfully", fg ="cyan", bg = "black")
    sl.pack(pady=1)

def back():
    my_root.destroy()

b2 = Button(text = "Back",command = back, width = 5, fg ="Black", bg= "Yellow")
b2.pack(side = BOTTOM,  pady =10)

b1 = Button(text = "Submit",command = submit, fg ="Black", bg= "Yellow")
b1.pack(side = BOTTOM)


my_root.mainloop()
